...
def time_convert(inp, mode, real_sd = '2010-01-02', sd_monday= "2009-12-28"):
    """
    Converts between different time formats and calculates temporal features.
    
    Parameters:
    - inp: Input time value
    - mode: Conversion type (e.g., 'e2t'=epoch to time, 't2e'=time to epoch)
    - real_sd: Real start date for week number calculations
    - sd_monday: Start date Monday for day number calculations
    
    Returns:
    Converted time value in the requested format
    """
    if mode == 'e2t':
...
def add_action_thisweek(act, columns, lines, act_handles, week_index, stop, firstdate, dname = 'r6.2'):
    thisweek_act = []
    while True:
        if not lines[act]: 
            stop[act] = 1
...
def combine_by_timerange_pandas(dname = 'r6.2'):
    """
    Combines user activity data from different sources (email, http, file, etc.)
    into weekly chunks for processing.
    
    Parameters:
    - dname: Dataset name/version
    
    Creates weekly pickle files containing combined activity data
    """
    ...
    while sum(stop.values()) < 5:
        ...
        for act in allacts:
            if 'email' == act:
                ...
                if dname in ['r6.2']: columns = ['id', 'date', 'user', 'pc', 'url/fname', 'content']
            elif 'file' == act:
                if dname in ['r6.2']: columns = ['id', 'date', 'user', 'pc', 'url/fname','activity','to','from','content']
...
def process_user_pc(upd, roles):
    """
    Determines primary PC assignments for users and identifies shared PCs.
    
    Parameters:
    - upd: User-PC mapping dataframe
    - roles: User roles information
    
    Returns:
    Updated dataframe with PC assignments and shared PC information
    """
    upd['sharedpc'] = None
...
def getuserlist(dname = 'r6.2', psycho = True):
    """
    Builds comprehensive user information list including roles, departments,
    and psychometric scores if available.
    
    Parameters:
    - dname: Dataset name/version
    - psycho: Whether to include psychometric scores
    
    Returns:
    DataFrame containing user information and organizational structure
    """
...
def get_mal_userdata(data = 'r6.2', usersdf = None):
    """
    Identifies malicious users and their activities from ground truth data.
    
    Parameters:
    - data: Dataset name/version
    - usersdf: Optional pre-loaded user dataframe
    
    Returns:
    DataFrame with malicious user information and activity timestamps
    """
...
def is_after_whour(dt):
    """
    Checks if a given timestamp is outside normal work hours (7:30-17:30).
    
    Parameters:
    - dt: Datetime object to check
    
    Returns:
    Boolean indicating if time is after work hours
    """
...
def is_weekend(dt):
    """
    Checks if a given timestamp falls on a weekend.
    
    Parameters:
    - dt: Datetime object to check
    
    Returns:
    Boolean indicating if date is weekend
    """
    if dt.strftime("%w") in ['0', '6']:
        return True
    return False   
    
def email_process(act, data = 'r6.2', separate_send_receive = True):
    """
    Extracts features from email activities including recipient counts,
    external communications, attachments, and text characteristics.
    
    Parameters:
    - act: Email activity record
    - data: Dataset version
    - separate_send_receive: Whether to separate send/receive features
    
    Returns:
    List of extracted email features
    """
...
def http_process(act, data = 'r6.2'): 
    """
    Extracts features from HTTP activities including URL characteristics,
    domain categories, and content analysis.
    
    Parameters:
    - act: HTTP activity record
    - data: Dataset version
    
    Returns:
    List of extracted HTTP features
    """
    # basic features:
...
def file_process(act, complete_ul = None, data = 'r6.2', filetype = 'act'):
    """
    Extracts features from file activities including file types,
    locations, and operations.
    
    Parameters:
    - act: File activity record
    - complete_ul: Complete user list
    - data: Dataset version
    - filetype: Type of file activity
    
    Returns:
    List of extracted file features
    """
    if filetype == 'act':
...
def from_pc(act, ul):
    """
    Determines the relationship between user and PC used for activity.
    
    Parameters:
    - act: Activity record
    - ul: User list with PC assignments
    
    Returns:
    Tuple of (PC type code, PC identifier)
    PC type codes: 0=own PC, 1=shared PC, 2=other's PC, 3=supervisor's PC
    """
...
def process_week_num(week, users, userlist = 'all', data = 'r6.2'):
    """
    Processes all user activities for a given week into numerical features.
    
    Parameters:
    - week: Week number to process
    - users: User information dataframe
    - userlist: List of users to process
    - data: Dataset version
    
    Creates pickle files containing processed numerical features
    """
...
def get_sessions(uw, first_sid = 0):
    """
    Groups user activities into sessions based on logon/logoff events.
    
    Parameters:
    - uw: User activity data for the week
    - first_sid: Starting session ID
    
    Returns:
    Dictionary of session information including timing and activities
    """
    sessions = {}
    open_sessions = {}
    sid = 0
...
def get_u_features_dicts(ul, data = 'r6.2'):
    """
    Creates mapping dictionaries for categorical user features.
    
    Parameters:
    - ul: User list dataframe
    - data: Dataset version
    
    Returns:
    Tuple of (updated user list, feature dictionaries, feature names)
    """
    ufdict = {}
...
def proc_u_features(uf, ufdict, list_f = None, data = 'r6.2'): #to remove mode
    if type(list_f) != list:
...
def f_stats_calc(ud, fn, stats_f, countonly_f = {}, get_stats = False):
    """
    Calculates statistical features for a group of activities.
    
    Parameters:
    - ud: User data
    - fn: Feature names
    - stats_f: Features to compute statistics for
    - countonly_f: Features to only count occurrences
    - get_stats: Whether to compute detailed statistics
    
    Returns:
    Tuple of (count, computed features, feature names)
    """
    f_count = len(ud)
    r = []
    f_names = []
...
def f_calc_subfeatures(ud, fname, filter_col, filter_vals, filter_names, sub_features, countonly_subfeatures):
    """
    Calculates features for filtered subsets of activities.
    
    Parameters:
    - ud: User data
    - fname: Feature name prefix
    - filter_col: Column to filter on
    - filter_vals: Values to filter for
    - filter_names: Names for filtered subsets
    - sub_features: Features to compute for each subset
    - countonly_subfeatures: Features to only count in subsets
    
    Returns:
    Tuple of (computed features, feature names)
    """
...
def f_calc(ud, mode = 'day', data = 'r6.2'):
    """
    Main feature calculation function that computes all behavioral features
    for a given set of user activities.
    
    Parameters:
    - ud: User activity data
    - mode: Analysis mode ('day', 'week', or 'session')
    - data: Dataset version
    
    Returns:
    List of [activity count, weekend flag, computed features, feature names, malicious flag]
    """
...
def to_csv(week, mode, data, ul, uf_dict, list_uf, subsession_mode = {}):
    """
    Exports processed features to CSV files.
    ...
    - data: Dataset version
    - ul: User list
    - uf_dict: User feature dictionaries
    - list_uf: List of user features
    - subsession_mode: Configuration for session subdivision
    
    Creates CSV files containing extracted features
    """
...
if __name__ == "__main__":
    """
    Main execution block that runs the complete feature extraction pipeline:
    """
